...
func TestStudentID(t *testing.T) {
	...
	t.Run(`student_id pattern is not true`, func(t *testing.T) {
		...
		g.Expect(err.Error()).To(Equal(fmt.Sprintf("StudentID: %s does not validate as matches(^[BMD]\\d{7}$)", Student.StudentID)))
	})
...
func TestPhoneNumber(t *testing.T) {
	...
	t.Run(`phone_number check 10 digit`, func(t *testing.T) {
		...
		g.Expect(ok).NotTo(BeTrue())
		g.Expect(err).NotTo(BeNil())
		g.Expect(err.Error()).To(Equal(fmt.Sprintf("Phone: %s does not validate as stringlength(10|10)", Student.Phone)))
